...
const val DEMO_GHOST_SESSION_ID = "DEMO_KESSELBERG_GHOST_V1"
...
/** Personal-best ghost for the demo session — same route, ~22% more lean, ~10% faster, 10 days ago. */
fun createDemoGhostSession(): RideSession {
    val baseTime = System.currentTimeMillis() - 10L * 24 * 3600 * 1000

    data class WP(val lat: Double, val lng: Double, val spd: Double, val lean: Float, val ms: Long)

    // Identical lat/lng to createDemoSession so corner centroids match within the 0.0003° threshold.
    // Lean scaled ×1.22 (capped ±50°), speed ×1.10 (capped 130 km/h), duration ×0.87 (faster lap).
    val wps = listOf(
        WP(47.6022, 11.3487,  90.0,   0f,  6100),
        WP(47.6068, 11.3508,  62.0,  33f,  4350),
        WP(47.6098, 11.3494,  79.0,   0f,  3480),
        WP(47.6132, 11.3520,  51.0, -46f,  5650),
        WP(47.6170, 11.3499,  97.0,   0f,  4780),
        WP(47.6208, 11.3526,  64.0,  28f,  4350),
        WP(47.6248, 11.3504,  55.0, -40f,  5220),
        WP(47.6295, 11.3489, 106.0,   0f,  5220),
        WP(47.6330, 11.3516,  46.0, -50f,  6520),
        WP(47.6360, 11.3495,  59.0,  37f,  4780),
        WP(47.6395, 11.3524,  99.0,   0f,  5220),
        WP(47.6442, 11.3500,  70.0,  30f,  4350),
        WP(47.6475, 11.3528,  94.0,   0f,  4780),
        WP(47.6508, 11.3504,  57.0, -34f,  4780),

        WP(47.6478, 11.3465, 108.0,   0f,  5220),
        WP(47.6445, 11.3442,  66.0,  41f,  4780),
        WP(47.6408, 11.3464,  51.0, -45f,  5220),
        WP(47.6368, 11.3445, 121.0,   0f,  5650),
        WP(47.6328, 11.3466,  59.0,  35f,  4350),
        WP(47.6288, 11.3444,  55.0, -38f,  4780),
        WP(47.6248, 11.3464, 101.0,   0f,  4780),
        WP(47.6208, 11.3444,  66.0,  26f,  4350),
        WP(47.6172, 11.3462,  84.0,   0f,  3910),
        WP(47.6138, 11.3444,  55.0, -33f,  4350),
        WP(47.6105, 11.3462,  88.0,   0f,  3480),
        WP(47.6065, 11.3444,  70.0,  29f,  3910),
        WP(47.6035, 11.3462,  86.0,   0f,  4350),
        WP(47.6022, 11.3487,  60.0,   0f,  2610),
    )
    ...
    for (i in 0 until wps.lastIndex) {
        ...
        repeat(steps) { step ->
            ...
            val lng = from.lng + (to.lng - from.lng) * frac
            val lean = when {
                abs(to.lean) > 8f && abs(from.lean) < 8f -> to.lean * sin(frac * PI).toFloat()
                abs(from.lean) > 8f && abs(to.lean) < 8f -> from.lean * (1f - sin(frac * PI / 2).toFloat())
                abs(to.lean) > 8f -> from.lean + (to.lean - from.lean) * frac
                else -> from.lean * (1f - frac)
            }
            val speed = if (abs(to.lean) > 10f)
                from.spd + (to.spd - from.spd) * frac.toDouble().pow(2.0)
            else
            ...
            val dlng = (lng - (prevPt?.longitude ?: lng)) * 111320.0 * cos(Math.toRadians(lat))
            points.add(TelemetryPoint(
                timestamp = t, latitude = lat, longitude = lng,
                speedKmh = speed.coerceIn(30.0, 130.0),
                leanAngle = lean.coerceIn(-50f, 50f),
                pitchAngle = if (speed > 90 && abs(lean) < 6f) 1.8f else 0f,
                distanceDelta = sqrt(dlat * dlat + dlng * dlng)
            ))
            t += stepMs
        }
    }
    ...
    val corners = mutableListOf<CornerEvent>()
    var cId = 0; var inCorner = false; var cStart = 0; var maxAbs = 0f; var maxIdx = 0
    for (i in points.indices) {
        val absLean = abs(points[i].leanAngle)
        if (!inCorner && absLean > 10f) { inCorner = true; cStart = i; maxAbs = absLean; maxIdx = i }
        else if (inCorner) {
            if (absLean > maxAbs) { maxAbs = absLean; maxIdx = i }
            if (absLean < 7f || i == points.lastIndex) {
                val slice = points.subList(cStart, i)
                val isLeft = points[maxIdx].leanAngle < 0
                corners.add(CornerEvent(
                    id = cId++, startIndex = cStart, endIndex = i,
                    maxLeftLean  = if (isLeft) -maxAbs else 0f,
                    maxRightLean = if (!isLeft) maxAbs else 0f,
                    maxLeanIndex = maxIdx,
                    centroidLat = slice.map { it.latitude }.average(),
